...
def create_local_fs(size_kb, block_device, data_path, mount_path):
    dd_cmd = 'dd if=/dev/zero of="%s" bs=1024 count=%d' % (data_path, size_kb)
...
def write_until_usage(block_device, min_file_bytes, max_file_bytes, target_usage_ratio):
    ...
    print('total: %d, target_usage: %d, current: %d' % (total, target_usage, usage))

    if usage < target_usage:
        while usage < target_usage - 10000:
            file_size = random.randint(min_file_bytes, max_file_bytes)
...
block_device = run_command('losetup -f').std_out.strip()
data_path = 'test.ext4'
mount_path = '/media/test.ext4'

try:
    create_local_fs(100000, block_device, data_path, mount_path)

    for i in range(1):
        target_usage_ratio = random.random()
        write_until_usage(block_device, 1000, MEGABYTE, target_usage_ratio)

    run_command_streaming_stdout(['../build/src/dj_cmd', block_device, '/'])
